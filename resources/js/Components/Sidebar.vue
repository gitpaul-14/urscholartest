<template>
    <div class="lg:max-h-screen bg-white flex flex-col border-r" id="side-bar" :class="dataOpenSideBar == true ? 'side-bar-visible' : 'side-bar-close'">
      ...
      <!-- mobile -->
      <div class="lg:hidden flex flex-row items-center justify-between bg-white border-b p-2">
        <div class="flex items-center">
          <div v-show="dataOpenSideBar" class="menu-man text-left whitespace-nowrap ml-2 text-xl font-bold text-black">
            <!-- Sidebar Content -->
            <div class="py-1 rounded-sm cursor-pointer text-blue-900 hover:bg-gray-100 hover:rounded-md">
              <Link :href="(route('coordinator.dashboard'))" class="flex items-center space-x-2 font-quicksand font-semibold pl-2 text-[16px]">
                  <span class="material-symbols-rounded" style="color: #0D47A1;" v-tooltip.right="!dataOpenSideBar ? 'Dashboard' : ''">
                  dashboard
                  </span>
        ...
        <div class="flex">
          <!-- Additional Items for Horizontal Layout -->
        </div>
      </div>
    </div>
  </template>
